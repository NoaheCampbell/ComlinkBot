...
async function scrapeData(url) {
    ...
    await page.screenshot({ path: 'abilityScreenshot.png', fullPage: true });

    const abilityHtml = await page.content();

    // Simulates clicking through the browser to get to the inventory
    await page.$eval('.ct-quick-nav__toggle.ct-quick-nav__toggle--closed', form => form.click()); // Opens up quick nav menu
    // await page.waitForSelector('ct-quick-nav__menu');
    await page.$$eval('.ct-quick-nav__button-label', (elements, searchText) => {
        const targetElement = elements.find(e => e.textContent.includes(searchText));
        if (targetElement) {
            targetElement.click();
        }
    }, 'Inventory'); // Pass 'Inventory' as the searchText argument    const inventoryHtml = await page.content();
    await page.screenshot({ path: 'inventoryScreenshot.png', fullPage: true });
    ...
    characterData = readDataWithRetry(abilityHtml, characterData);

    // Return the populated characterData
    return characterData;
}

function formatSkillCheck(skillName, characterData) {
    const check = characterData.skillChecks[skillName];
    const reroll = characterData.skillRerolls[skillName];
    return `${skillName}: ${check}${reroll !== 'flat' ? ', ' + reroll : ''}`;
}
...
function readData(html, characterData) {
    ...
    skillElements.each((index, element) => {
        ...
        const skillBonusRaw = $(element).find('.ddbc-signed-number').attr('aria-label').trim();
        const skillBonus = skillBonusRaw.replace(/\s+/g, ''); // Removes all whitespace
        const skillKey = convertSkillNameToKey(skillName);

        characterData.skillChecks[skillKey] = skillBonus;

        // Initialize reroll status to 'flat'
        characterData.skillRerolls[skillKey] = 'flat';

        // Check for advantage or disadvantage
        const advantageElement = $(element).find('.ddbc-tooltip.ct-skills__adjustment.ddbc-advantage-icon.ddbc-tooltip--dark-mode');
        if (advantageElement.length) {
            const modifier = advantageElement.attr('data-original-title');
            if (modifier === 'Advantage') {
                characterData.skillRerolls[skillKey] = 'Advantage';
            } else if (modifier === 'Disadvantage') {
                characterData.skillRerolls[skillKey] = 'Disadvantage';
            }
        }

        // Add a sign to the bonus if it's not present
        // const formattedBonus = skillBonus.startsWith('+') || skillBonus.startsWith('-') ? skillBonus : `+${skillBonus}`;
        // characterData.skillChecks[skillKey] = formattedBonus;
    });
...
Skill Checks:
    ${formatSkillCheck('Acrobatics', characterData)}
    ${formatSkillCheck('AnimalHandling', characterData)}
    ${formatSkillCheck('Arcana', characterData)}
    ${formatSkillCheck('Athletics', characterData)}
    ${formatSkillCheck('Deception', characterData)}
    ${formatSkillCheck('History', characterData)}
    ${formatSkillCheck('Insight', characterData)}
    ${formatSkillCheck('Intimidation', characterData)}
    ${formatSkillCheck('Investigation', characterData)}
    ${formatSkillCheck('Medicine', characterData)}
    ${formatSkillCheck('Nature', characterData)}
    ${formatSkillCheck('Perception', characterData)}
    ${formatSkillCheck('Performance', characterData)}
    ${formatSkillCheck('Persuasion', characterData)}
    ${formatSkillCheck('Religion', characterData)}
    ${formatSkillCheck('SleightOfHand', characterData)}
    ${formatSkillCheck('Stealth', characterData)}
    ${formatSkillCheck('Survival', characterData)}
`});
            }
        }
        ).catch((err) => {
